/**
 * 世界观群聊页侧边栏组件（会话列表 + 右键菜单 + 桌面折叠 / 移动端抽屉）
 *
 * 与 chat-sidebar 结构相同，但使用世界观专属 API（API.world.getSessions）。
 ...
 *   worldId         — String   世界观 ID（用于获取该世界的群聊会话列表）
 *
 * Emits:
 ...
 *   close()                — 关闭移动端抽屉
 *   refresh()              — 重新从服务端拉取 sessions（世界观专属）
...
const WorldSidebar = {
    ...
    props: {
        ...
        /** 世界观 ID。为 null 时跳过 refresh()，等加载完成后再调用。 */
    ...
    methods: {
         ...
         */
        goSettings: function () {
            window.location.href = API.BASE_PATH + 'world_settings.html';
        },
        ...
        goCharacterChat: function () {
            window.location.href = API.BASE_PATH + 'character_index.html';
        },
        ...
        refresh: async function () {
            ...
            try {
                let result = await API.world.getSessions(this.worldId);
                if (result.status === 200) {
                    this.sessions = result.data || [];
                }
            } catch (error) {
                console.error('获取世界观会话列表失败:', error);
            }
        },
