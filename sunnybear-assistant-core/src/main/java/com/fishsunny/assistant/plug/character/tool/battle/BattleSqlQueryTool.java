...
public class BattleSqlQueryTool implements ToolHandler {
    ...
    public BattleSqlQueryTool(ObjectMapper objectMapper) {
        ...
        register = new ToolRegister()
                .setName(NAME)
                .setDescription("""
                        对当前战斗会话的临时 SQLite 数据库执行查询（仅限 SELECT / PRAGMA / EXPLAIN 等只读语句）。\
                        返回 Markdown 格式的表格结果，最多 100 行。\
                        共有 6 张表：player_state / player_skills / player_buffs / enemy_state / enemy_skills / enemy_buffs。\
                        查询前可先用以下语句了解库中有哪些表：\
    ...
    public ToolExecutor.ToolExecuteResponse action(String argumentsJson, Map<String, Object> context) throws ToolExecutor.ToolExecuteException {
        ...
        if (ds == null) {
            throw new ToolExecutor.ToolExecuteException("当前没有活跃的战斗会话，无法查询战斗数据库");
        }
